...
<head>
    ...
    <title>Connexion Contrôleur - Tropi-Techno</title>
...
<body class="bg-gradient-to-br from-primary/20 to-secondary/20 font-inter min-h-screen flex items-center justify-center p-4">
    <div class="max-w-md w-full animate-fade-up">
        <div class="text-center mb-8">
            <div class="inline-flex items-center justify-center w-20 h-20 bg-white rounded-2xl shadow-lg mb-4 pulse-slow"><i class="fas fa-clipboard-list text-primary text-4xl"></i></div>
            <h1 class="text-3xl font-bold text-dark">Tropi-Techno</h1><p class="text-gray-600 mt-1">Espace Contrôleur</p>
        </div>
        
        <div class="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden">
            <div class="bg-gradient-to-r from-primary to-secondary px-6 py-4"><h2 class="text-xl font-semibold text-white">Connexion</h2><p class="text-white/80 text-sm">Accédez à votre espace contrôleur</p></div>
            
            <form action="{{ route('controleur.login.submit') }}" method="POST" class="p-6">
                @csrf
                @if($errors->any())<div class="bg-red-50 border-l-4 border-red-500 text-red-700 p-3 mb-4 rounded text-sm">{{ $errors->first() }}</div>@endif
                ...
                <div class="mb-5">
                    ...
                    <div class="relative">
                        ...
                        <input type="email" name="email" required value="{{ old('email') }}" class="w-full pl-10 pr-3 py-3 border border-gray-300 rounded-xl focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition" placeholder="controleur@tropitechno.com">
                    </div>
                </div>
                ...
                <div class="mt-6 pt-4 text-center border-t"><p class="text-xs text-gray-500"><i class="fas fa-shield-alt mr-1 text-green-500"></i>Connexion sécurisée - Accès réservé au personnel autorisé</p></div>
            </form>
        </div>
