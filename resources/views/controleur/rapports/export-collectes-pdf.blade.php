...
<body>
    ...
    <table>
        <thead><tr><th>Code</th><th>Date</th><th>Producteur</th><th>Produit</th><th>Quantité</th><th>Montant</th></tr></thead>
        <tbody>
            @foreach($collectes as $c)
            <tr><td>{{ $c->code_collecte }}</td><td>{{ $c->date_collecte->format('d/m/Y') }}</td><td>{{ $c->producteur->nom_complet }}</td><td>{{ $c->produit }}</td><td class="text-right">{{ number_format($c->quantite_nette) }} kg</td><td class="text-right">{{ number_format($c->montant_total, 0, ',', ' ') }} CFA</td></tr>
            @endforeach
        </tbody>
    </table>
