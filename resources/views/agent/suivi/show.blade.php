    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        <div class="bg-gradient-to-r from-primary to-secondary px-6 py-4">
            <div class="flex justify-between items-center">
                ...
                <span class="px-3 py-1 bg-white/20 rounded-full text-white text-sm">{{ $suivi->sante_cultures }}</span>
            </div>
        </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p>{{ number_format($suivi->superficie_actuelle, 2) }} hectares</p>
                </div>
                <div>
            ...
            <div class="mt-4 p-3 bg-red-50 rounded-lg">
                <label class="text-red-700 text-sm font-semibold">Problèmes constatés</label>
                <p class="text-red-600 mt-1">{{ $suivi->problemes_constates }}</p>
            </div>
            @endif
            
            @if($suivi->recommandations)
            <div class="mt-4 p-3 bg-green-50 rounded-lg">
                <label class="text-green-700 text-sm font-semibold">Recommandations</label>
                <p class="text-green-600 mt-1">{{ $suivi->recommandations }}</p>
            </div>
            @endif
        </div>
    </div>
